...
public class KThread {
	 ...
	 */
	public ThreadState schedulingState = null;
	...
	// To keep track of the waitQueue start time
	private long waitStartTime = Long.MAX_VALUE;
	...
	public void join() {
		...
		// 0. Do I set a variable - join Called so that - when called again, we
		// don't do the following steps?

		Lib.debug(dbgThread, "Joining to thread: " + toString());
		// 1. Check if current thread is not the thread it's waiting for
		Lib.assertTrue(this != currentThread);

		// 2. Check the status of the thread, if its finished
		if (this.status == statusFinished) {
			return;
		}

		// 3. 'this' thread is certainly not in running status, this will either
		// be waiting for a resource in a waiting queue in blocked state or
		// might be in the ready queue. Anyways, we just want to wait till this
		// thread reaches a state of completion.

		// 4. 'Current thread' needs to wait in the join queue and needs to go
		// into a blocked state
		// A. Disable interrupts
		boolean intStatus = Machine.interrupt().disable();
		// B. Add thread to joinQueue
		joinQueue.waitForAccess(currentThread());

		// Put the current thread to sleep until the status moves to finished
		while (this.status == statusFinished)
			currentThread().sleep();

		// Must check when the thread of execution comes here
		Lib.assertTrue(this.status == statusFinished);

		KThread thread;
		// Once over, move all the threads waiting here to ready?
	...
	public void setWaitStartTimestamp(long waitStartTime) {
		this.waitStartTime = waitStartTime;
	}

	public long getWaitStartTimestamp() {
		return waitStartTime;
	}

	public long resetWaitStartTimestamp() {
		return waitStartTime = Long.MAX_VALUE;
	}
